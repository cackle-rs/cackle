...
struct ExeInfo<'input> {
    ...
    /// Mapping from symbol definitions to source locations. This is used as a fallback when we
    /// can't get the source location for a relocation. In practice, this ends up being used when
    /// variable data contains references. e.g. `GET_PID` in `test_crates/crab4/src/lib.rs`.
    symbol_to_locations: HashMap<Symbol, SourceLocation>,
}
...
pub(crate) fn scan_objects(
    ...
    let symbol_to_locations = dwarf::get_symbol_to_locations(&dwarf)?;
